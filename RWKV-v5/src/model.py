...
class RWKV_TimeMix_RWKV6(MyModule):
    def __init__(self, args, layer_id):
        super().__init__()
        self.args = args
        self.layer_id = layer_id
        ...
        with torch.no_grad():
            ...
            # fancier, data-dependant time_mix
            self.time_mix_k_w1 = nn.Parameter(torch.empty(args.n_embd, args.time_mix_extra_dim).uniform_(-0.01, 0.01))
            self.time_mix_k_w2 = nn.Parameter(torch.zeros(args.time_mix_extra_dim, args.n_embd))
            self.time_mix_v_w1 = nn.Parameter(torch.empty(args.n_embd, args.time_mix_extra_dim).uniform_(-0.01, 0.01))
            self.time_mix_v_w2 = nn.Parameter(torch.zeros(args.time_mix_extra_dim, args.n_embd))
            self.time_mix_r_w1 = nn.Parameter(torch.empty(args.n_embd, args.time_mix_extra_dim).uniform_(-0.01, 0.01))
            self.time_mix_r_w2 = nn.Parameter(torch.zeros(args.time_mix_extra_dim, args.n_embd))
            self.time_mix_g_w1 = nn.Parameter(torch.empty(args.n_embd, args.time_mix_extra_dim).uniform_(-0.01, 0.01))
            self.time_mix_g_w2 = nn.Parameter(torch.zeros(args.time_mix_extra_dim, args.n_embd))
...
class RWKV_ChannelMix_RWKV6(MyModule):
    def __init__(self, args, layer_id):
        super().__init__()
        self.args = args
        self.layer_id = layer_id
...
class Block(nn.Module):
    def __init__(self, args, layer_id):
        ...
        if 'v6' in os.environ["RWKV_MY_TESTING"]:
            attn_class = RWKV_TimeMix_RWKV6
            ffn_class = RWKV_ChannelMix_RWKV6
        else:
            attn_class = RWKV_TimeMix_RWKV5
            ffn_class = RWKV_ChannelMix

        if self.layer_id == 0 and self.args.pre_ffn > 0:
            self.ffnPre = ffn_class(args, 0)
        else:
            self.att = attn_class(args, layer_id)

        if 'g' in os.environ["RWKV_MY_TESTING"]:
            self.ffn = MishGLU(args, layer_id)
        else:
            self.ffn = ffn_class(args, layer_id)
